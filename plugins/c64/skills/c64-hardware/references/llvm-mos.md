...
Calls through function pointers, or into assembly the compiler can't see, are assumed to call anything. Inline assembly counts as assembly the compiler can't see. On zig-mos 0.17 (LLVM 22), a function that contains any inline assembly, volatile or not, with or without a `memory` clobber, makes the callers above it that keep locals across the call move their frames to the soft stack, and an `extern` callee does the same. In one game, putting the two screen-cell routines into inline assembly grew eleven caller functions by 755 bytes in total and made a status-bar update 830 cycles slower (9%), more than the faster cell loops saved. zig-mos offers no `leaf` or `nonreentrant` attribute to prevent it. Put hand-written assembly in a routine whose callers are few and cold, and compare the callers' sizes, and whether they index `(__rc0),y`, before and after. `-fnonreentrant` (or `__attribute__((nonreentrant))`) declares code non-reentrant when function pointers would otherwise defeat the analysis. `__attribute__((leaf))` on an assembly routine tells the compiler it calls no C code. `-fno-static-stack` turns the whole mechanism off.
...
- **Check every branch in the linked binary.** On zig-mos 0.17, a relative branch in inline assembly whose target lies more than 128 bytes away assembled and linked without an error, and landed at the wrapped address. This happened twice in one routine. Once, a guard branch jumped 42 bytes before the routine's start. Another time, a loop-back branch that six added bytes put out of range jumped past the routine's end. Disassemble the ELF and check that every branch target lies inside its routine, and write `beq +; jmp far` for long jumps.
- **Don't clobber a pinned input.** With an input pinned to Y (`"{y}"`) and `.y = true` in the clobbers, the compiler dropped the `ldy` that loads the input. Declare a register the block changes as an output instead (`[out] "={y}" (dummy)`).
- **Name tables by an assembly symbol.** An `"i"` operand holding a Zig global's address prints the global's IR name, such as `gfx.row_low`, but the object defines it as `.Lgfx.row_low`, so the link fails with an undefined symbol. `export const` fails too, because LTO internalises the unreferenced export. Define a table that inline assembly indexes in module-level `asm` with `.globl` and `.p2align`, generating its bytes at compile time. A table of up to 2^k bytes aligned to 2^k never crosses a page, so `.p2align 5` keeps a 25-byte row table's `abs,x` reads at 4 cycles.
- **Self-modified operands work.** Numeric labels let a block patch its own operands (`sta 1f+1` … `1: sta $FFFF,y`). Code runs from RAM on the C64, and the block rewrites the operands before each use, so a warm restart finds nothing stale. Host tests can't run such code, so prove it against its reference on the machine.
...
- **Assembly.** Clobbers are a packed struct of flags, such as `.{ .a = true, .x = true, .p = true, .memory = true, .rc10 = true }` (fields `memory`, `a`, `x`, `y`, `s`, `c`, `n`, `v`, `z`, `p` and `rc0`–`rc255`; never clobber `rc0`/`rc1` or `rc30`/`rc31`). An `r` operand lands in an imaginary register, usable as `(%[ptr]),y`, and `{a}`, `{x}` and `{y}` pin an operand to a register. See section 6 for the traps. `.incbin` paths resolve relative to the build cache, so generate the `.s` file from `build.zig` with an absolute path. `@embedFile` refuses paths outside the module's directory.
